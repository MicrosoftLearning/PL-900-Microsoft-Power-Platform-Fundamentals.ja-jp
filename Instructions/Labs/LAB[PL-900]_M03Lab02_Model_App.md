...
- "従業員経費管理" という名前の新しいモデル駆動型アプリを作成します。
...
1. 画面の右側の **[プロパティ]** パネルで、**"表示名"** フィールドを選択し、それを "`Report Information`" に変更します
...
1. **[所有者]** を選択します。 [プロパティ] パネルで、**[ラベル]** を `Requestor` に変更します
...
1. **[名前]** を `Expense Reports Due Today` に変更し、 **[保存]** を選択します。
...
1. **[名前]** に「`Employee Expense Management`」と入力して、**[作成]** を選びます。
...
1. 画面の右側にある **[表示オプション]** セクションで、**[タイトル]** プロパティを `Reports` に変更します
