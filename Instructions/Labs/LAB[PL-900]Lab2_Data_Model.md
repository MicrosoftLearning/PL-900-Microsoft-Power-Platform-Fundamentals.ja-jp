...
2.  Power Pages のホーム画面で、左側のナビゲーションを使用して **[テーブル]** を選択します。
3.  **[テーブル]** で、**[Copilot の使用を開始する]** を選択します。
4.  **[Copilot で構築するテーブルについて説明する]** 画面で、次のように入力します。「イベントを管理するためのテーブルを作成してください。** テーブルは、イベントの名前、イベント データ、場所、出席者の最大数、およびイベントの詳細を識別する必要があります。」
5.  **[生成]** ボタンの横にある **[テーブル設定]** を選択し、次のように構成します。
    - **テーブル オプション:** 1 つのテーブル
...
> **重要:** Copilot によって作成されるテーブルは、おそらく Events という **1 つ**のテーブルのみです。 それより多く作成された場合は、削除するテーブルの名前を Copilot に指定して、それらを **DELETE** する必要があります。 **余分なテーブルを削除しないと、次の手順に影響します。**
...
7.  **[次は何をしますか]** フィールドに次のように入力します。「Event Type という名前の選択肢の列を追加してください」。**
8.  次のテキストを入力して、別の列を追加します: 「Registration Required という名前の選択肢の列を追加してください。」**
...
9.  上部の**コマンド バー**で、**[+ 既存のテーブル]** を選択します。
10.  **[検索]** フィールドに「**Contact**」と入力し、**[選択した項目を追加]** を選択します。
...
11.  **[次は何をしますか]** フィールドに次のように入力します。「Event Session という新しいテーブルを追加してください。」**

Copilot はおそらく、Event Session と Event Speaker の 2 つのテーブルを作成します。 Contacts は話者となるため、Event Speaker テーブルは削除します。

12.  必要に応じて、**[次は何をしますか]** フィールドに次のように入力します。「Event Speaker テーブルを削除してください。」**
13.  Copilot に、次のテキストを入力します。「Event Session テーブルに Session Description という新しいテキスト列を追加してください。」**

![追加された Event Session テーブルを示すスクリーンショット](media/546162a8b040a7bdcdcd5c3e2be44b4d.png)

次に、Session Registrations という最後のテーブルを追加します。 このテーブルは、特定のセッションに登録する個人を管理するために使用されます。

14.  Copilot に、次のテキストを入力します。「Session Registrations という新しいテーブルを追加してください。」**
...
15.  Copilot に、次のテキストを入力します。「Participant テーブルを削除してください。」**
    
場合によっては、セッション登録テーブルに Participant Name 列が追加されることがあります。 後でデータ モデルを保存するときに問題が発生する可能性があるため、これを削除する必要があります。 (後で別の参加者列に置き換える予定です。)  

16. 必要に応じて、Copilot に次のテキストを入力します。「Session Registration テーブルから Participant Name 列を削除してください。」

17. Primary Column フィールドがある場合は、次のテキストを入力します。Primary Column の名前を Registration Name に変更してください。**

18.  Copilot に、次のテキストを入力します。「Session Registration テーブルに Special Instructions というテキスト列を追加してください。」**
...
18.  コマンド バーで、**[リレーションシップの作成]** を選択します。
19.  関連付けを次のように構成します。
- **リレーションシップの種類:** 一対多
- **1:** 連絡先
- **多:** Event Session
...
21.  **コマンド バー**で、**[リレーションシップの作成]** を選択します。
22.  関連付けを次のように構成します。
- **リレーションシップの種類:** 一対多
- **1:** 連絡先
...
24.  **コマンド バー**で、**[リレーションシップの作成]** を選択します。
25.  関連付けを次のように構成します。
- **リレーションシップの種類:** 一対多
- **1:** イベント
- **多:** Event Session
...
27.  **コマンド バー**で、**[リレーションシップの作成]** を選択します。
28.  関連付けを次のように構成します。
- **リレーションシップの種類:** 一対多
- **1:** Event Session
...
3.  **[検索]** フィールドに「**Event**」と入力します。
4.  **Event** テーブルを開きます。
5.  **[スキーマ]** 見出しの **[列]** を選択します。
6.  **Event Type** 列を見つけて開きます。
7.  ラベルを次のものに置き換えます。
    - 会議
    - 見本市
    - ウェビナー
    - 昼食および学習
    - リリース
8.  **[既定の選択肢]** を **[なし]** に設定します。

![更新された Event Type の選択肢のスクリーンショット。 ](media/3fb723bad1c0fced1e3705124a2d4594.png)

9.  **[保存]** ボタンを選択します。 (初回で、列の保存が失敗する場合は、もう一度試してください。)**

次に、セッション登録の合計数を追跡するために、**Event Session** テーブルに新しい列を追加します。

10.  左側のナビゲーションを使用して **[テーブル]** を選択し、**Event** テーブルから移動します。
11.  **[検索]** フィールドに「**Event**」と入力します。
12.  **Event Session** テーブルを開きます。
13.  **[スキーマ]** 見出しの **[列]** を選択します。
14.  コマンド バーで、**[新しい列]** ボタンを選択します。
15.  新しい列を次のように構成します。
...
2.  **[テーブル]** で、**[Copilot の使用を開始する]** を選択します。
3.  **[Copilot で構築するテーブルについて説明する]** 画面で、次のように入力します。「機器をチェックアウトするためのテーブルを作成してください。** テーブルには、Equipment Name、Due Date、Item number を含める必要があります。」
4.  **[生成]** ボタンの横にある **[テーブル設定]** を選択し、次のように構成します。
- **テーブル オプション:** 1 つのテーブル
- リレーションシップを**含めない**でください。
...
テーブルを作成したので、次にテーブルにいくつかの列を追加します。 まず、イベントの種類を表す列を追加します。 また、イベントには連絡先を付け加える予定です。 既存の Contact テーブルをデータ モデルに追加し、後でイベント登録に関連付けます。
6.  **[次は何をしますか]** フィールドに次のように入力します。「テーブル名を Equipment に変更してください。」**
7.  **[次は何をしますか]** フィールドに次のように入力します。「Equipment type という選択肢の列を追加してください。」**
8.  次のテキストを入力して、別の列を追加します: 「Category というテキスト列を追加してください。」**
9.  次のテキストを入力して、別の列を追加します: 「Status というテキスト列を追加してください。」**

完成したテーブルは次の画像のようになります。

![Equipment テーブルのスクリーンショット。](media/Equipment-table-checkpoint.png)

10. テーブルに余分な列がある場合は、「[列名] 列を削除してください」と入力して、それらの列を削除します。**
11. テーブルが画像と一致したら、**[保存して終了]** を選択します。

## タスク 4:Equipment テーブルを直接変更する

1.  必要に応じて、Web ブラウザーを開いて Power Apps Maker Portal に移動し、Microsoft アカウントの資格情報を使用してサインインします。
2.  左側のナビゲーションを使って、**[テーブル]** を選択します。
3.  **[検索]** フィールドに「**Equipment**」と入力します。
4.  **Equipment** テーブルを開きます。
5.  **[スキーマ]** 見出しの **[列]** を選択します。
6.  **Equipment Type** 列を見つけて開きます。
7.  必要に応じて、ラベルを次のように置き換えます。
- 電子
- 家具
- ツール
- アクセサリ
