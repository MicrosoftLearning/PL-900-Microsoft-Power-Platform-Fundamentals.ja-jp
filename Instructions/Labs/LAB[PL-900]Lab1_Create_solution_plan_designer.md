...
2.  **[計画を作成する]** で次のテキストを入力します。"スタッフに機器を貸し出し、期限をお知らせしてもらう必要があります。"**
...
6.  **[ユーザーの要件]** 見出しの **[Copilot]** フィールドに次のように入力します。"機器が貸し出されている間に、機器に問題がある場合は報告できるようにする必要があります。"**
7.  **[移動]** ボタンを選択します。
...
11.  画面右側の **[Copilot]** ペインで、次のように入力します。"機器テーブルにアイテム番号という名前のテキスト列を追加します。"**
12.  列が**機器**テーブルに追加されたら、**Copilot** を使用して、次のプロンプトを個別に入力します。
    1.  "スタッフ/従業員テーブルにマネージャーという名前の新しいテキスト列を追加します。"
    2.  "機器の種類と言う名前の機器テーブルに新しい選択肢の列を追加します。"
    3.  "機器の種類と言う名前の機器テーブルに新しい選択肢の列を追加します。 "
...
16.  **[Copilot]** フィールドの [テクノロジ] 見出しの下に、次のように入力します。"従業員の質問に応答するアイテム貸し出しエージェントを作成します。"**
17.  **[移動]** ボタンを選択します。
...
14. **Text** プロパティを次のように変更します。"新規の予約を開始するか、既存の予約を表示します。"
15. **エンティティの予約/貸し出し**のテキストを選択します。
16. Text プロパティを "機器の予約" に変更します。
17. 画面の上部にある **[スタート画面]** テキストを選択します。
18. **Text** プロパティを "Contoso 機器貸し出し" に変更します。
...
2.  **[エージェントのテスト]** ペインで、次のように入力します。*"Bob Johnson には期限超過のアイテムが貸し出されていますか?"*
