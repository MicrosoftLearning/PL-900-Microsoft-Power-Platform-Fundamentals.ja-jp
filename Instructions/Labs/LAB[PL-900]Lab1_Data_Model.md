...
1. **[編集]** を選択し、**[+ 新規]** 行を選択 (または最初の空の行を選択) し、次のサンプル レコードを入力します。
...
1.  左側のナビゲーション ウィンドウで、**テーブル**を選択します。
1.  **[テーブル]** で、**[空白のテーブルを使用して開始]** を選択します。
1.  テーブルの名前を **Table1** から **Room** に変更します。

    ![テーブルの名前変更を示すスクリーンショット](media/f43bbdbb9e05fe968b5f6cd301ad6b46.png)

1. 次に、**New Column** の名前を **Room Name** に変更します。
   - Copilot ペインで、テキスト `Rename New Column in the Room table to Room Name` を入力します。
...
5.  **Copilot** ペインで、上記のすべてのテキスト列を追加します。
    - テキスト `Add new columns named Campus, Building, and Floor to the room table.` を入力します。

    ![Copilot を使用して項目を追加する方法を示すスクリーンショット](media/712f56af13c331924442a04dce14cddb.png)

1.  **Copilot** ペインで、**Conference Room** という**はい/いいえ**列を追加します。
    
    - テキスト `Add a new yes/no column named Conference Room to the Room table` を入力します。
...
8. **[保存して終了]** ボタンを選択して新しい "部屋テーブル" を作成します。

1. **[作業を終了しましたか?]** ダイアログ ボックスで、**[保存して終了]** を選択します。
