...
2.  「**新規**」をクリックして、「**自動化**、「**クラウド フロー**」を選択してから、「**自動**」を選択します。これにより、Power Automate フロー エディターが新規ウィンドウで開きます。

3. 「**フォローのトリガーの選択**」で、「**Microsoft Dataverse**」を検索します。

4. トリガー「**行が追加、変更、削除されたとき**」を選択してから、「**作成**」をクリックします。
...
9. 「*メール*」を検索して、「**Office 365 Outlook**」コネクタおよび「**メールの送信 (V2)**」アクションを選択します。
...
2. 「**新規**」をクリックして、「**自動化**、「**クラウド フロー**」を選択してから、「**スケジュール済み**」を選択します。これにより、Power Automate フロー エディターが新規ウィンドウで開きます。

3. **15** 分ごとに繰り返すようにフローを設定します。
...
10.  電子メールによる通知を送信する
     ...
     * 「各ループに適用」で「**アクションの追加**」をクリックします。「**メールの送信 (V2)**」アクションを「**Office 365 Outlook**」接続から追加します。
...
13.  **本文**フィールドに次の情報を入力します。**名前** は、**構築ステップの取得**からの動的コンテンツです。一覧の下部までスクロールする必要がある場合があります。
